alter table public.reparaciones
  add column if not exists descuento_aplicado_manual boolean not null default false;
...
begin
  ...
  if not found
     ...
     or (new.created_at < cliente.descuento_disponible_desde and not coalesce(new.descuento_aplicado_manual, false)) then
    return new;
  end if;
